...
Report confidence interval around IHC model 
```{r}
summary(model.ihc.endless)
tidy(model.ihc.endless,conf.int=TRUE,exponentiate=F,effects="fixed") %>%
  kable(digits=3)
tidy(model.ihc.endless,conf.int=TRUE,exponentiate=T,effects="fixed") %>%
  kable(digits=3)
```


Report confidence interval around next number model 
```{r}
summary(model.nn.endless)
tidy(model.nn.endless,conf.int=TRUE,exponentiate=F,effects="fixed") %>%
  kable(digits=2)
# exponentiated for OR
tidy(model.nn.endless,conf.int=TRUE,exponentiate=T,effects="fixed") %>%
  kable(digits=3)
```


Report confidence interval around Productivity model 
```{r}
summary(model.prod.endless)
tidy(model.prod.endless,conf.int=TRUE,exponentiate=F,effects="fixed") %>%
  kable(digits=3)
# exponentiated for OR
tidy(model.prod.endless,conf.int=TRUE,exponentiate=T,effects="fixed") %>%
  kable(digits=3)
```
...
Full model vs base (sig)
```{r}
anova(large.endless.full, base.endless, test = 'LRT')
```

Full model vs. 1-factor models (n.s.)
```{r}
# full model vs. initial models
anova(large.endless.full, model.ihc.endless, test = 'LRT')
anova(large.endless.full, model.nn.endless, test = 'LRT')
anova(large.endless.full, model.prod.endless, test = 'LRT')
```
...
tidy(model.nn.infinity, conf.int=TRUE,exponentiate=F,effects="fixed") %>%
  kable(digits=3)
## exponentiate for OR
tidy(model.nn.infinity, conf.int=TRUE,exponentiate=T,effects="fixed") %>%
  kable(digits=3)
```
