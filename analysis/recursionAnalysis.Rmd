...
wcn.data %>%
  mutate(WithinOutsideIHC = factor(WithinOutsideIHC, levels = c("within", "outside"), 
                                   labels = c("Within IHC", "Outside IHC")))%>%
  dplyr::filter(TaskType == "immediate") %>%
  dplyr::group_by(Productivity, WithinOutsideIHC, LadlabID, prod.gradient) %>%
  dplyr::summarize(meansubj = mean(Accuracy, na.rm = TRUE)) %>%
  ...
  scale_colour_brewer(guide=FALSE, palette="Set1", direction=-1) + 
...
```{r regression-graphs}
# Successor knowledge
fullmodel.sf.prod <- glm(SuccessorKnower ~ Productivity + Highest_Contig_NN.c + 
                              IHC.c + Age.c, 
                            family = "binomial", data = distinct_model.df)
...
# Endless knowledge
fullmodel.end.prod <- glm(EndlessKnower ~ Productivity + Highest_Contig_NN.c + 
                              IHC.c + Age.c, 
                            family = "binomial", data = distinct_model.df)
summary(fullmodel.end.prod)
fullmodel.end.gain <- glm(EndlessKnower ~ prod.gradient + Highest_Contig_NN.c + 
                              IHC.c + Age.c, 
                            family = "binomial", data = distinct_model.df)
