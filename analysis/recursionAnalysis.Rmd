...
###MODEL BUILDING AND COMPARISONS###
#base model for successor knower
subset_base.model <- glmer(SuccessorKnower ~ Age + (1|LadlabID), family = "binomial", 
                    data = subset_distinct_model.df)
##IHC model##
subset_model.ihc.sf <- glmer(SuccessorKnower ~ IHC + Age + (1|LadlabID), family = "binomial", 
                   data = subset_distinct_model.df)
#convergence warnings, is this an issue?
with(subset_model.ihc.sf@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#True, so we're okay

#compare
anova(subset_base.model, subset_model.ihc.sf, test = 'LRT') #IHC not significant

##Highest NN Model##
subset_model.nn.sf <- glmer(SuccessorKnower ~ Highest_Contig_NN + Age + (1|LadlabID), family = "binomial", 
                   data = subset_distinct_model.df)

#compare
anova(subset_base.model, subset_model.nn.sf, test = 'LRT')#highest contiguous NN not significant

##Productivity model##
subset_model.prod.sf <- glmer(SuccessorKnower ~ Productivity + Age + (1|LadlabID), family = "binomial", 
                   data = subset_distinct_model.df)

#compare
anova(subset_base.model, subset_model.prod.sf, test = 'LRT')#Productivity trending
```
