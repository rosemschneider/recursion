...
```{r}
unique.nn <- as.vector(wcn.wide$LadlabID)
...
model2a.sf <- glmer(SuccessorKnower ~ mean.NN + Age + (1|LadlabID), family = "binomial", 
                   data = distinct_model.df)
anova(base.model, model2a.sf, test = 'LRT')#highest NN not significant
...
ggplot(model.df, aes(x = IHC, y = EndlessKnower)) +
  geom_point()

hist(model.df$EndlessKnower)

#okay with about mean NN
model2.endless <- glmer(EndlessKnower ~ mean.NN + Age + (1|LadlabID), family = "binomial", 
                        data = distinct_model.df)
anova(model2.endless, base.endless, test = 'LRT')
```