...
# opts_knit$set(root.dir = "~/Projects/Junyi study/recursiongithub/") #for PC
>>>>>>> 86508d1a4ef7323fb95eb161e68df277cd72415a
library(Hmisc)
library(tidyverse)
library(magrittr)
#library(langcog)
...
<span style="color:red">Question from RMS: is FHC capped at 100 as well? Yes, capping it at 100 for now - PC. Junyi - thoughts? </span>
...
wcn_model.df <- wcn_score.df %>%
  mutate(Productivity = factor(Productivity, levels = c(0,1)), 
         TrialType = factor(WithinOutsideIHC, levels = c(0,1)))%>%
...
<<<<<<< HEAD
##add highest contig
large.endless.nn <- glmer(EndlessKnower ~ Highest_Contig_NN.c + IHC.c + Age + (1|LadlabID), 
=======
##IHC v. Highest contiguous##
...
<<<<<<< HEAD
##add productivity
large.endless.full <- glmer(EndlessKnower ~ Productivity + Highest_Contig_NN.c + IHC.c + Age + (1|LadlabID), 
=======
##Productivity v. Highest contiguous##
large.endless.prod2 <- glmer(EndlessKnower ~ Productivity + IHC.c + (1|LadlabID), 
                          family = "binomial", data = distinct_model.df)

#compare
anova(large.endless.base, large.endless.prod2, test = 'LRT')##Productivity is significant
...
>>>>>>> 86508d1a4ef7323fb95eb161e68df277cd72415a
                          family = "binomial", data = distinct_model.df)
#convergence warnings, is this an issue?
with(large.endless.full@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#True, we're okay

#compare 
anova(large.endless.nn, large.endless.full, test = 'LRT')
...
large.endless.base <- glmer(EndlessKnower ~ IHC.c + Age + (1|LadlabID), 
                            family = "binomial", data = distinct_model.df)
##add prod
large.endless.prod <- glmer(EndlessKnower ~ Productivity + IHC.c + Age + (1|LadlabID), 
                          family = "binomial", data = distinct_model.df)

#compare these two
anova(large.endless.base, large.endless.prod, test = 'LRT')

#compare with full
anova(large.endless.prod, large.endless.full, test = 'LRT')
```
...
large.endless.prod.base <- glmer(EndlessKnower ~ Productivity + Age + (1|LadlabID), 
                            family = "binomial", data = distinct_model.df)
## prod + IHC
large.endless.prod.ihc <- glmer(EndlessKnower ~ Productivity + IHC.c + Age + (1|LadlabID), 
                          family = "binomial", data = distinct_model.df)

#compare
anova(large.endless.prod.base, large.endless.prod.ihc, test = 'LRT')
```
...
large.endless.prod.base <- glmer(EndlessKnower ~ Productivity + Age + (1|LadlabID), 
                            family = "binomial", data = distinct_model.df)
## prod + HCNN
large.endless.prod.hcnn <- glmer(EndlessKnower ~ Productivity + Highest_Contig_NN.c + Age+ (1|LadlabID), 
                          family = "binomial", data = distinct_model.df)

# compare
anova(large.endless.prod.base, large.endless.prod.hcnn, test = 'LRT')

# compare with full
anova(large.endless.prod.hcnn, large.endless.full, test = 'LRT')
