...
fig3.model.df <- fig3.data %>%
  ...
  mutate(IHC.c = as.vector(scale(IHC, center = TRUE, scale=TRUE)), #scale and center for model fit
         ...
         TaskItem.c = as.vector(scale(TaskItem_num, center=TRUE, scale=TRUE)))
str(fig3.model.df)
...
fig3.model.base <- glmer(Accuracy ~ age.c + TaskItem.c + (1|LadlabID), 
                        family = "binomial", data = fig3.model.df)
fig3.model.ihc <- glmer(Accuracy ~ IHC.c + age.c + TaskItem.c + (1|LadlabID), 
                       family = "binomial", data = fig3.model.df)
fig3.model.prod <- glmer(Accuracy ~ prod.group + IHC.c + age.c + TaskItem.c +
                     (1|LadlabID), family = "binomial", data = fig3.model.df)
fig3.model.noint <- glmer(Accuracy ~ prod.group + TaskItem_type+ IHC.c + age.c +
                            TaskItem.c +
                     (1|LadlabID), family = "binomial", data = fig3.model.df)
fig3.model.full <- glmer(Accuracy ~ prod.group + TaskItem_type +
                           prod.group:TaskItem_type + IHC.c + age.c + TaskItem.c +
                     (1|LadlabID), family = "binomial", data = fig3.model.df)
#comparison of models
anova(fig3.model.full, fig3.model.noint, fig3.model.prod, fig3.model.ihc, fig3.model.base, test = 'LRT')
...
fig3.model2.df <- fig3.data %>%
  ...
  mutate(IHC.c = as.vector(scale(IHC, center = TRUE, scale=TRUE)), #scale and center for model fit
         FHC.c = as.vector(scale(FHC, center = TRUE, scale=TRUE)), 
         age.c = as.vector(scale(Age, center = TRUE, scale=TRUE)),
         TaskItem.c = as.vector(scale(TaskItem, center=TRUE, scale=TRUE)))
str(fig3.model2.df)
...
wcn.model.base <- glmer(Accuracy ~ age.c + TaskItem.c + (1|LadlabID), 
                        family = "binomial", data = wcn_model.df)
wcn.model.ihc <- glmer(Accuracy ~ IHC.c + age.c + TaskItem.c + (1|LadlabID), 
                       family = "binomial", data = wcn_model.df)
...
Regression tables are saved in a separate Excel sheet for easier formatting. Here we try a plot of betas.
```{r regression-graphs}
# visualization
library(effects)
