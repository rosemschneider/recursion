...
fig3.data %>%
  ...
  theme(legend.position = "right", legend.title = element_blank(), 
        panel.grid.minor = element_blank()) +
  labs(x = "Task item", y = "Average proportion correct") +
...
wcn.data %>%
  mutate(WithinOutsideIHC = factor(WithinOutsideIHC, levels = c("within", "outside"), 
                                   labels = c("Within IHC", "Beyond IHC")), 
         Productivity.tertiary = factor(Productivity.tertiary, levels = c("Nonproductive", "Productive (IHC ≥ 99)", 
                                                                          "Productive (IHC < 99)")))%>%
  dplyr::filter(TaskType == "immediate") %>%
  dplyr::group_by(Productivity.tertiary, WithinOutsideIHC, LadlabID, prod.gradient) %>%
  dplyr::summarize(meansubj = mean(Accuracy, na.rm = TRUE)) %>%
...
anova(wcn.model2.base, wcn.model2.ihc, wcn.model2.range, wcn.model2.noint, wcn.model2.int, test = 'LRT')
```
