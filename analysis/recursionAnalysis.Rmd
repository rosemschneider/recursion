...
# kid's data were NA for wcn practice trials nd 0 correct on test. not sure why prac were marked as NA but assume kid fail wcn.
...
<span style="color:red">Note practice trials 1 and 5 are included in highest contig NN computation but originally excluded in overall percent correct, or within vs. beyond. Added back 1 and 5 in %corr and within vs beyond just so the measures are comparable- PC</span>
...
```{r}
wcn.data %>%
  dplyr::filter(TaskType == "immediate" | TaskType == "practice") %>%
...
##add productivity
large.endless.prod <- glmer(EndlessKnower ~ Productivity + Highest_Contig_NN.c + IHC + (1|LadlabID), 
                          family = "binomial", data = distinct_model.df)
#convergence warnings, is this an issue?
