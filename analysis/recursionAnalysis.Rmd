...
Percent Correct on WCN (note this includes 1 and 5)
```{r}
wcn.wide %>%
  group_by(LadlabID) %>%
...
```{r}
wcn.wide %>%
  group_by(Productivity, WithinOutsideIHC, LadlabID) %>%
  summarize(meansubj = mean(immediate, na.rm = TRUE)) %>%
...
corrdf <- full.data %>%
  right_join(highest_contiguous_nn) %>%
  distinct(LadlabID, Highest_Contig_NN, Age, IHC, FHC) %>%
  select(-LadlabID)
