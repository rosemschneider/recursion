...
Question from RMS: is FHC capped at 100 as well?
```{r}
...
full.data <- right_join(full.data, hc.data, by = "LadlabID")
...
full.data %<>%
  dplyr::rename(Productivity = prod_tomerge, 
                ...
                FHC = fhc_tomerge, 
                DCE = dce)%>%
  mutate(Productivity = factor(Productivity, levels = c("nonprod", "prod"), 
...
full.data %>%
  ...
  droplevels()%>%
  distinct(LadlabID, Response, Productivity) %>%
  mutate(Response = factor(Response, levels = c("nonprod", "prod"), 
                           labels = c("Nonproductive", "Productive")))%>%
  mutate(changed_classification = ifelse((is.na(Response) & Productivity == "Nonproductive"),
                                         "NA_toNonprod",
  ...
  group_by(changed_classification)%>%
  summarise(n =n())
...
Finally, same data by productivity
```{r}
full.data %>%
  distinct(LadlabID, Productivity, FHC, IHC, DCE)%>%
  group_by(Productivity)%>%
  summarise(mean_IHC = mean(IHC),
            sd_IHC = sd(IHC),
            mean_FHC = mean(FHC),
...
Note from RMS: This doesn't look right based on a histogram of the IHC data, but I'm moving onto analyses with a flag to fix this later, because I'm not quite sure what's going wrong - looks like there might be weirdness translating between count (proportion) and frequency??
```{r}
unique.hc.data <- full.data %>%
  distinct(LadladID, Gender, Age, AgeGroup, HCReceivedSupport, IHC, DCE, FHC, Productivity)

ggplot(unique.hc.data, aes(x=IHC, colour = Productivity)) + 
...
Note from RMS: make sure this doesn't have the same issue as above
```{r}
...
ggplot(unique.hc.data, aes(x=Age, colour = Productivity)) + 
  geom_dotplot(aes(fill = Productivity),
               binwidth = .03,
...
Note from RMS: I left sup.times out when selecting data, will have to add back in, just too lazy to do it rn
```{r}
full.data %>%
...
```{r}
#Need to pull productivity into wcn.wide data
lookup <- full.data %>%
  filter(TaskType == "productivity")%>%
  distinct(LadlabID, Response)
...
wcn.wide <- add_productivity()

wcn.wide %>%
  group_by(WithinOutsideIHC, Prod_Class) %>%
  summarize(mean = mean(immediate, na.rm = TRUE), 
            sd = sd(immediate, na.rm = TRUE))
```
...
wcn.wide %>%
 dplyr:: select(-momentum)%>%
  group_by(Prod_Class, WithinOutsideIHC)%>%
  summarise(n = n())
```
...
distinct_model.df <- model.df %>%
  distinct(LadlabID, Age, AgeGroup, Gender, SuccessorKnower, EndlessKnower, 
           IHC, Highest_NN, Highest_Contig_NN)%>%
...
with(model1.sf@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#True, so no issues

#compare
anova(base.model, model1.sf, test = 'LRT') #IHC not significant

#highest nn
model2.sf <- glmer(SuccessorKnower ~ Highest_Contig_NN + Age + (1|LadlabID), family = "binomial", 
                   data = distinct_model.df)
#convergence warnings, is this an issue?
...
#what about mean NN
model2a.sf <- glmer(SuccessorKnower ~ mean.NN + Age + (1|LadlabID), family = "binomial", 
                   data = distinct_model.df)
anova(base.model, model2a.sf, test = 'LRT')#ns
...
model2.endless <- glmer(EndlessKnower ~ Highest_Contig_NN + Age + (1|LadlabID), family = "binomial", 
                        data = distinct_model.df) 
#convergence warnings, is this an issue?
with(model2.endless@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#True, model converging

anova(model2.endless, base.endless, test = 'LRT')#Highest contig NN significant
