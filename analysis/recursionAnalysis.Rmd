...
In addition, we will remove kids who failed the What Comes Next Task practice trials.
```{r eval=FALSE}
...
# put "exclude" under "include" for the following kids
exclude.wcn <- data.frame(LadlabID = c("022616-JM","030216-AD","031516-A",
                           "041316-AR","041316-VN","032216-RC","012316-BO"))

# exclude.wcn$LadlabID <- as.character(exclude.wcn$LadlabID)
# full.data$LadlabID <- as.character(full.data$LadlabID)
...
full.data %>%
  filter(Productivity == "Productive") %>%
  distinct(LadlabID, HCReceivedSupport, IHC, FHC) %>%
  group_by(HCReceivedSupport) %>%
  summarize(mean = mean, na.rm=TRUE),
            sd = sd(Response, na.rm=TRUE),
```
